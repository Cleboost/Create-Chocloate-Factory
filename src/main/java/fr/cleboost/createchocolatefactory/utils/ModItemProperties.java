package fr.cleboost.createchocolatefactory.utils;

import fr.cleboost.createchocolatefactory.item.utils.ChocolateProgressItem;
import net.minecraft.client.renderer.item.ItemProperties;
import net.minecraft.resources.ResourceLocation;
import net.minecraft.world.item.Item;

public class ModItemProperties {
    public static void addCustomItemProperties() {
        makeChocolateProgressItem(ModItems.BARS.get());
    }

    public static void makeChocolateProgressItem(Item item) {
        ItemProperties.register(item, new ResourceLocation("eatprogress"), (pItemStack, pClientLevel, pEntity, pInt) -> {
            return pEntity != null && pItemStack.is(ModItems.BARS.get()) ? ChocolateProgressItem.getEatProgress(pItemStack) : 0;
        });
    }
}
