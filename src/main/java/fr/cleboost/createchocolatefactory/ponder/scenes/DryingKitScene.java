...
public class DryingKitScene {
    public static void scene(SceneBuilder scene, SceneBuildingUtil util) {
        scene.title("drying_kit", CCFBlocks.DRYING_KIT.asItem().getDescription().getString());
        ...
        scene.overlay().showControls(centerVec, Pointing.DOWN, 40).rightClick()
                .withItem(new ItemStack(CCFItems.COCOA_BEANS_WET.get(), 9));
        ...
        scene.world().createItemEntity(centerVec, new Vec3(0, 0, 0), new ItemStack(CCFItems.COCOA_BEANS_DIRTY.get(), 9));
